...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (isUselessTrade)
            r += 1024; // Smirnov penalty: Reduce search depth for lazy trades
            
        if (isFischerCapture)
            r -= 1024; // Fischer bonus: Search deeper when cleaning our half
            
        if (!capture && type_of(movedPiece) == PAWN)
            r -= 512;  // Karpov bonus: Search deeper for pawn structure / space gaining moves
