...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (depth <= 0)
        return qsearch<PvNode ?
        PV : NonPV>(pos, ss, alpha, beta);
    ...
    ttData.value = ttHit ?
    value_from_tt(ttData.value, ss->ply, pos.rule50_count()) : VALUE_NONE;
    ...
    {
        // If ttMove is quiet, update move sorting 
        // heuristics on TT hit
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        // Move List.
        // In MultiPV mode we also skip PV moves that have been already
        ...
        // Custom heuristics flags for search modification
        bool isFischerCapture = false;
        bool isUselessTrade = false;
        bool isTalSacrifice = false;

        if (capture)
        {
            Square toSq = move.to_sq();
            Piece capturedPc = pos.piece_on(toSq);
            Rank toRank = rank_of(toSq);
            Square oppKingSq = pos.square<KING>(~us);
            // 1. Fischer Rule: If opponent piece is on our half (Ranks 1-4 for White, 5-8 for Black)
            if ((us == WHITE && toRank <= RANK_4) || (us == BLACK && toRank >= RANK_5)) {
                isFischerCapture = true;
            }

            // 2. Smirnov Rule: Averse lazy trades (equal material trades that are not Fischer captures)
            if (!isFischerCapture && type_of(capturedPc) == type_of(movedPiece)) {
                isUselessTrade = true;
            }

            // 3. Tal Rule: Identify potential sacrifices near the opponent's king (distance <= 2)
            int dist_x = std::abs(int(file_of(toSq)) - int(file_of(oppKingSq)));
            int dist_y = std::abs(int(rank_of(toSq)) - int(rank_of(oppKingSq)));
            if (std::max(dist_x, dist_y) <= 2 && type_of(movedPiece) >= KNIGHT) {
                isTalSacrifice = true;
            }
        }
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!isTalSacrifice // Custom override: Tal sacrifices bypass SEE pruning
        ...
        // If all moves but one
        // fail low on a search of (alpha-s, beta-s), and just one fails high on
        // (alpha, beta), then that move is singular and should be extended.
        // To
        ...
        if (allNode)
            r += r / (depth + 1);
        // Custom LMR (Late Move Reduction) modifications
        if (isUselessTrade)
            r += 1024;
        // Smirnov penalty: Reduce search depth for lazy trades
            
        if (isFischerCapture)
            r -= 1024;
        // Fischer bonus: Search deeper when cleaning our half
            
        if (!capture && type_of(movedPiece) == PAWN)
            r -= 512;
        // Karpov bonus: Search deeper for pawn structure / space gaining moves
        ...
        // If a stop occurred, the return value of
        ...
        if (rootNode)
        {
            ...
            rm.averageScore =
              rm.averageScore != -VALUE_INFINITE ?
            ...
            if (moveCount == 1 || value > alpha)
            {
                ...
                // move position in the list is preserved - just the PV is pushed 
                // up.
    ...
    // If we are in a singular extension search then
...
// depth zero, or recursively with further decreasing depth.
// With depth <= 0, we
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    // the moves.
    // We presently use two stages of move generator in quiescence search:
        ...
        && type_of(pos.captured_piece()) >= ROOK)
    {
              ...
              & ~pos.pieces()))  // no pawn pushes available
        {
            ...
            // search for legal king-moves only
...
// "plies to mate from the current position".
...
Move Skill::pick_best(const RootMoves& rootMoves, size_t multiPV) {
    ...
    // Choose best move.
    // For each move score we add two terms, both dependent on
    // weakness.
    // One is deterministic and bigger for weaker levels, and one is
    // random.
...
void SearchManager::check_time(Search::Worker& worker) {
    ...
    std::min(512, int(worker.limits.nodes / 1024)) : 512;
    ...
    if (
      ...
      && ((worker.limits.use_time_management() && (elapsed > tm.maximum() || stopOnPonderhit))
        ...
        worker.threads.stop 
= worker.threads.abortedSearch = true;
}

// Used to correct and extend PVs for moves that have a TB (but not a mate) score.
// Keeps the search based PV for as long as it is verified to maintain the game
// outcome, truncates afterwards.
// Finally, extends to mate the PV, providing a
...
void syzygy_extend_pv(const OptionsMap&         options,
    ...
    // with TB support.
    // However, it can be that a position is draw due to the 50 move
    // rule if it has been been reached on the board with a non-optimal 50 move counter
    // (e.g. 8/8/6k1/3B4/3K4/4N3/8/8 w - - 54 106 ) which TB with dtz counter rounding
    // cannot always correctly rank.
    // See also
    // https://github.com/official-stockfish/Stockfish/issues/5175#issuecomment-2058893495
    // We adjust the score to match the found PV.
    // Note that a TB loss score can be
